...
RCP = 'RCP45' # cambiar ruta para cada RCP (RCP45, RCP60, RCP85)
path = r'D:\Pyto_AGUAMET_UPO\EscenariosCC\RRNN\TMIN' + '\\' + RCP + '\\'
fileExt = r".csv"
...
serie_rcp = open(path + 'Evento_' + RCP +'_tmin.csv', 'w') 
...
for j in range(len(stationselect)):
    ...
    serie_rcp.write(seriestr + '\n')
serie_rcp.close()        

RCP = 'RCP60' # cambiar ruta para cada RCP (RCP45, RCP60, RCP85)
path = r'D:\Pyto_AGUAMET_UPO\EscenariosCC\RRNN\TMIN' + '\\' + RCP + '\\'
fileExt = r".csv"
stations = pd.read_csv(r'D:\Pyto_AGUAMET_UPO\Scripts' + '\\Stations_CC_AEMET.csv', sep =';') # puntos a extraer e imprimir en fichero de evento
stationsid = pd.DataFrame(stations['ID'].unique())
